...
function autoAdjustTextareaHeight (textarea, dragGrid) {
  ...
  const correctHeight = rows * textareaRowHeight
  textarea.style.height = correctHeight + 'px'
...
const DragAndDrop = {
  ...
  dragStart: function (section, eventX, eventY) {
    ...
    section.setAttribute(
      DragAndDrop.dragOffsetLeftAttribute,
      sectionOffsetLeft - eventX
    )
    section.setAttribute(
      DragAndDrop.dragOffsetTopAttribute,
      sectionOffsetTop - eventY
    )
    section.style.position = 'absolute'
    section.style.left = sectionOffsetLeft + 'px'
    section.style.top = sectionOffsetTop + 'px'
  },
  drag: function (section, eventX, eventY) {
    const offsetLeft = parseInt(
      section.getAttribute(DragAndDrop.dragOffsetLeftAttribute)
    )
    const offsetTop = parseInt(
      section.getAttribute(DragAndDrop.dragOffsetTopAttribute)
    )
  ...
  dragEnd: function (section, eventY) {
    ...
    const sections = Array.from(
      sectionsElement.querySelectorAll('.section:not(.section-drag)')
    )
...
const Elements = {
  ...
  editDiv: function () {
    const editDiv = BaseElements.div(
      'hcontainer stretch full-width section-edit'
    )
  ...
  section: function () {
    ...
    deleteButton.onclick = function (event) {
      ...
      if (deleteButton.classList.contains('section-delete-clicked')) {
        ...
        Array.from(
          document.getElementsByClassName('section-delete-clicked')
...
function initialize () {
  ...
  window.addEventListener('mousemove', event => {
    const draggedSections = Array.from(
      document.getElementsByClassName('section-drag')
    )
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker
      .register('/todo/service_worker.js')
      .then(registration => {
        registration.update()
      })
  }
